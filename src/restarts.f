...
!     
     ...
     &     orab,ielorien,trab,inotr,amname,amequ,
     &     amta,namta,t0,t1,iamt1,veold,
     ...
     &     key,prestr,iprestr,cbody,ibody,xbody,nbody,xbodyold,ttime,
     &     qaold,cs,mcs,output,physcon,ctrl,typeboun,iline,ipol,inl,
     &     ipoinp,inp,fmpc,tieset,ntie,tietol,ipoinpc,nslavs,t0g,t1g,
     &     nprop,ielprop,prop,mortar,nintpoint,ifacecount,islavsurf,
     &     pslavsurf,clearini,ier,vel,nef,velo,veloo,ne2boun,heading,
     &     network,irestartread,nfc,ndc,coeffc,ikdc,edc,xmodal,ndmat_,
     &     ndmcon,dmcon,dam,irefineloop)
!     
      implicit none
!     
...
!     
     ...
     &     nelemload(*),iamload(*),nelcon(*),ipoinpc(0:*),iendset(*),
     ...
     &     ne2boun(2,*),ndmat_,ndmcon(*),irefineloop
!     
...
!     
      do i=2,n
        if(textpart(i)(1:4).eq.'READ') then
          if(irefineloop.eq.1) then
            write(*,*) '*ERROR in restarts: *REFINE MESH'
            write(*,*) '       cannot be used in a restart'
            write(*,*) '       calculation'
            call exit(201)
          endif
          irestartread=2
...
!     
     ...
     &       nmpc,nalset,nmat,ntmat_,npmat_,norien,nam,nprint,mi,
     &       ntrans,ncs_,namtot,ncmat_,mpcfree,maxlenmpc,ne1d,ne2d,
     &       nflow,nlabel,iplas,nkon,ithermal,nmethod,iperturb,nstate_,
     ...
     &       xload,elcon,nelcon,rhcon,nrhcon,alcon,nalcon,alzero,
     &       plicon,nplicon,plkcon,nplkcon,orname,orab,
     &       ielorien,trab,inotr,amname,amequ,
     &       amta,namta,t0,t1,iamt1,veold,
     &       ielmat,matname,prlab,prset,filab,vold,nodebounold,
     &       ndirbounold,xbounold,xforcold,xloadold,t1old,eme,
     &       iponor,xnor,knor,thicke,offset,iponoel,inoel,rig,
     &       shcon,nshcon,cocon,ncocon,ics,sti,ener,xstate,jobnamec,
     &       infree,irestartstep,prestr,iprestr,cbody,ibody,xbody,nbody,
     &       xbodyold,ttime,qaold,cs,mcs,output,physcon,ctrl,typeboun,
     &       fmpc,tieset,ntie,tietol,nslavs,
